-- =====================================================================
-- Classifly.in — Seed Data (idempotent)
-- =====================================================================
...
INSERT INTO categories (slug, name, vertical, depth, is_leaf, display_order, name_i18n) VALUES
  ('mobiles',         'Mobiles',                'goods',       0, false, 10, '{"hi":"मोबाइल","ta":"மொபைல்","bn":"মোবাইল"}'),
...
-- ---------- CATEGORY ATTRIBUTES (a few examples) ----------
...
UPDATE feature_flags SET payload = '{"days":60}'::jsonb WHERE key = 'listing.expiry_days' AND payload = '{}'::jsonb;


-- =====================================================================
-- DEMO LISTINGS (only run in dev — comment out for prod seed)
-- =====================================================================
-- These rely on auth.users having at least one user; they would be created
-- via the seed_demo_users() function below. Wrap in DO block so the seed
-- file still applies cleanly on a prod env where we want no demo data.
...
DECLARE
  ...
  cat_car     uuid;
BEGIN
  -- Only seed demos if there are any users at all (dev env)
  IF (SELECT count(*) FROM auth.users) = 0 THEN
    RAISE NOTICE 'No auth users yet; skipping demo listings. Seed users via Supabase Auth then re-run.';
    RETURN;
  END IF;

  SELECT id INTO demo_seller FROM profiles LIMIT 1;
  SELECT id INTO bangalore   FROM cities WHERE slug = 'bengaluru';
  SELECT id INTO cat_mobile  FROM categories WHERE slug = 'mobile-phones' AND vertical = 'goods';
  SELECT id INTO cat_car     FROM categories WHERE slug = 'cars' AND vertical = 'goods';

  -- Demo: iPhone 13
  INSERT INTO listings (seller_id, category_id, vertical, title, description, price, price_type, condition, city_id, location, status, posted_at, expires_at, language)
  VALUES (demo_seller, cat_mobile, 'goods', 'iPhone 13 128GB Midnight - Excellent Condition', 'Used for 1 year, all accessories and original box included. No scratches.', 38000, 'negotiable', 'like_new', bangalore, ST_MakePoint(77.5946, 12.9716)::geography, 'active', now(), now() + interval '60 days', 'en')
  ON CONFLICT DO NOTHING;

  -- Demo: Maruti Swift
  INSERT INTO listings (seller_id, category_id, vertical, title, description, price, price_type, condition, city_id, location, status, posted_at, expires_at, language)
  VALUES (demo_seller, cat_car, 'goods', 'Maruti Swift VXI 2019 - Single Owner, 45,000 km', 'Single owner, all services on time at authorized dealer. Insurance valid till 2026.', 525000, 'negotiable', 'good', bangalore, ST_MakePoint(77.5946, 12.9716)::geography, 'active', now(), now() + interval '60 days', 'en')
  ON CONFLICT DO NOTHING;
END $$;
