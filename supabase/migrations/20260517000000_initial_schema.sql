-- =====================================================================
-- Classifly.in — Initial schema migration
-- =====================================================================
-- Mirror of docs/02-database/schema.sql.
-- This file is the *runtime* source of truth (applied by `supabase db push`).
-- The version in docs/ is the human-readable spec; both should match.
-- For schema CHANGES, always create a new dated migration here — never edit
-- this file after it's been applied to any environment.
-- =====================================================================
...
CREATE INDEX IF NOT EXISTS idx_listings_browse   ON listings(vertical, status, city_id, posted_at DESC) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_listings_category ON listings(category_id, status, posted_at DESC) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_listings_seller   ON listings(seller_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_listings_featured ON listings(is_featured, featured_until DESC) WHERE is_featured AND status = 'active';
CREATE INDEX IF NOT EXISTS idx_listings_fts      ON listings USING GIN(search_vector);
CREATE INDEX IF NOT EXISTS idx_listings_title_trgm ON listings USING GIN(title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_listings_geo      ON listings USING GIST(location) WHERE status = 'active';
CREATE INDEX IF NOT EXISTS idx_listings_expiry   ON listings(expires_at) WHERE status = 'active' AND expires_at IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_listing_attrs_gin ON listing_attributes USING GIN(attrs);
...
CREATE INDEX IF NOT EXISTS idx_apps_job       ON job_applications(job_id, status, applied_at DESC);
CREATE INDEX IF NOT EXISTS idx_apps_applicant ON job_applications(applicant_id, applied_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_subs_user_active ON subscriptions(user_id, status) WHERE status IN ('active','trialing');
...
LANGUAGE sql STABLE AS $$
  SELECT l.* FROM listings l
   ...
   ORDER BY l.is_featured DESC, l.boost_score DESC, l.posted_at DESC NULLS LAST
   LIMIT lim OFFSET off;
$$;


-- ---------- REALTIME PUBLICATION ----------
-- Allow clients to subscribe to chat updates in real time.
DO $$ BEGIN
